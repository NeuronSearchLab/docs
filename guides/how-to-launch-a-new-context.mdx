...
- Call `GET /v1/recommendations` with the new `context_id` and representative `user_id` values.
...
2. Team creates context `ctx_205` with checkout-safe filters.
3. Backend requests recommendations using `context_id=ctx_205` during checkout.
4. Events are tagged from checkout interactions and posted to `/v1/events`.

...
A backfill imports interaction history you already hold into NeuronSearchLab, so a model can train on it immediately.

The [embed](/embed) collects behaviour from the moment it renders, but only going forward. Where you already have a year of orders in Shopify or pageviews in GA4, importing that history means your first model trains on real customers rather than waiting a month for new behaviour to accumulate.

Whichever source you import from, the mapping is the same three questions: which item, which event, and when. The sections below cover each supported source.
...
Upload it under **Events → Import history**.
...
**You choose what each event name means.** Every export names its events differently, so nothing is guessed. After you pick the file, the console lists each distinct event name in it with its row count, and you choose which of your event IDs it becomes - or leave it out. `Order Completed` is not assumed to be a purchase, and `add_to_cart` is never folded into a click.

Your answers are saved, so the next export that uses the same names imports without asking again. Through the API, send the mapping as an `event_mapping` form field: a JSON object from each event name to an event ID, or `null` to leave it out. A request without a complete mapping starts nothing and answers with the names it found.
...
Order line items become verified purchases, on the event bound to the `purchase` signal - your workspace's **Purchase** event unless you have rebound it. Shopify does not expose product views, so this gives you conversions, which is the signal the embed cannot collect.
...
If you have linked GA4 to BigQuery, we can read per-user rows: every `view_item`, `select_item`, `add_to_cart` and `purchase` with the pseudonymous user id attached. **This trains a model.**

GA4's event names are a fixed schema, so they are translated once rather than asked about: `view_item` is the `view` signal, `select_item` is `click`, and `add_to_cart` and `purchase` are themselves. Each lands on the event ID bound to that signal, and starting the import creates any that are missing.
...
| Unknown event | An event name you left out, or one that was not mapped when the import started. |

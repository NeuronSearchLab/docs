...
**Goal:** use Studio to create one deterministic homepage outcome during a live weather event: the closures guide must appear in slot `2`.

### Step 1: open the right studio

1. Open **Console > Studio**.
2. Select the homepage weather emergency studio.
...
6. Save the rule as active.

### Step 3: preview the exact outcome

1. Open the **Preview** tab.
2. Enter a reader ID.
3. Run preview.

**Pass condition:** the item `School closures and travel disruption map` is in slot `2`.

### Step 4: verify why it happened

1. Open the **Verify** tab.
2. Enter the same item ID and user ID.
3. Run explain.

**Pass condition:** Verify shows the rule `Pin school closures guide to slot 2` matched the item.
...
2. Confirm:
   ...
   - no experiment is excluding the rule

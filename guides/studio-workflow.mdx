...
Select **Open in ChatGPT** above the composer to continue the workflow in the NeuronSearchLab ChatGPT plugin. If the composer contains text, the handoff includes that request. If it is empty, the handoff starts with a recommendation and ranking-explanation prompt.

The handoff opens the ChatGPT app when a compatible native handler is available and otherwise opens `chatgpt.com`. Install NeuronSearchLab from the ChatGPT plugin directory if it is not already available in your conversation. When ChatGPT asks you to connect, sign up or log in to the NeuronSearchLab console and approve access.

You do not need a local MCP server, Developer mode, an MCP URL, or an API key. See the [ChatGPT plugin setup steps](/sdk/mcp#chatgpt).

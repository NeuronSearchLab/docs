...
icon: 'sparkles'
description: 'Request personalized results from the API or official JavaScript and PHP SDKs.'
---

Once your catalog and events are flowing, you can begin requesting recommendations. This guide shows how to call the API directly and how to rely on the official JavaScript or PHP SDK for convenience. The `/recommendations` endpoint requires an access token that includes the `neuronsearchlab-api/read` scope.
...
Install the PHP SDK from Packagist and initialize it with an access token obtained from `/auth/token`:

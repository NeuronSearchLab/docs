...
Once your catalog and events are flowing, you can begin requesting recommendations. This guide shows how to call the API directly and how to rely on the official JavaScript or PHP SDK for convenience. The `/v1/recommendations` endpoint requires an access token that includes the `neuronsearchlab-api/read` scope.

## Choose a context and audience

Every request should specify the surface and user the results are meant for.

- **context_id** ties the request to a configuration defined in the console (for example `ctx_101` for Homepage Recommendations).
...
curl -X GET "https://api.neuronsearchlab.com/v1/recommendations" \
  ...
  --data-urlencode "context_id=ctx_101" \
...
- `context_id` (optional): prefixed reference to a stored context configuration that can introduce conditional filtering or grouping.

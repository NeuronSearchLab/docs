...
2. Map your data fields to the attributes accepted by the [`POST /v1/items` endpoint](/api-reference/endpoint/submit-items).
3. Use the [console Items page](https://console.neuronsearchlab.com/explore) for small batches or the API for automated pipelines.
...
1. Create a context for each surface and record the numeric console ID, such as `101`.
2. Configure the model family and pre-query filters inside the console under **Contexts**.
3. Reference that context ID in recommendation requests to ensure consistent behavior.
...
1. Send impressions, clicks, conversions, and other behavioral events using the [`POST /v1/events` endpoint](/api-reference/endpoint/submit-events).

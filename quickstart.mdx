...
With an access token you can request recommendations immediately. The following example retrieves homepage content for a known user using context ID `ctx_101`:

```bash
curl -X GET "https://api.neuronsearchlab.com/v1/recommendations?user_id=user-123&context_id=ctx_101&limit=10" \
...
1. Define event signals and save a template in **Events**
2. Start a training run from that template
3. Review the resulting trained version in **Models**
4. Add an optional label and description for your team
5. Approve it
6. Promote it to a serving target when you are ready to make it live

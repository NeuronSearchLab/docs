...
1. **Connect your catalog** - Send the item fields your app already knows, such as names, descriptions, and metadata.
2. **Capture real-world signals** - Track views, clicks, purchases, and custom events so recommendations can reflect real user behavior.
3. **Serve recommendations** - Ask for recommendations from your backend using the SDKs or Core API for the surface you are rendering.
4. **Tune quality over time** - Use contexts, rules, pipelines, model families, experiments, and analytics to improve the experience without changing your app every time.

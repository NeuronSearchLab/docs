...
Use `access_token` as the Bearer token for `/v1` API calls. `expires_in` is a lifetime in seconds; compute `expires_at = now + expires_in` in your token cache and refresh before that time.

...
The public API belongs behind your own server-side boundary. Do not expose OAuth client credentials, access tokens, or direct public API calls from a browser or mobile client.
...
Use the public API domain for resource calls and the auth domain for token exchange. The token endpoint is unauthenticated, lives on the auth subdomain, and is rate-limited independently from resource traffic.
